...
class Adm extends CI_Controller
{
	...
	public function m_siswa()
	{
		...
		if ($uri3 == "det") {
			...
			$ket 	= "";
			if ($p->id != 0) {
				$this->db->query("UPDATE m_siswa SET nama = '" . bersih($p, "nama") . "', nim = '" . bersih($p, "nim") . "', jurusan = '" . bersih($p, "jurusan") . "', id_jurusan = '" . bersih($p, "id_jurusan") . "'	WHERE id = '" . bersih($p, "id") . "'");
				$ket = "edit";
			} else {
				$ket = "tambah";
				$this->db->query("INSERT INTO m_siswa VALUES (null, '" . bersih($p, "nama") . "', '" . bersih($p, "nim") . "', '" . bersih($p, "jurusan") . "', '" . bersih($p, "id_jurusan") . "')");
			}
			$ret_arr['status'] 	= "ok";
			...
			foreach ($q_datanya as $d) {
				...
				$data_ok[] = $d['nama'];
				$data_ok[] = $d['nim'];
				$data_ok[] = $d['jurusan'] . '-' . $d['id_jurusan'];

				$data_ok[] = '<div class="btn-group">
                          <a href="#" onclick="return m_siswa_e(' . $d['id'] . ');" class="btn btn-primary btn-xs"><i class="glyphicon glyphicon-pencil" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Edit</a>
				...
				if ($d['ada'] == "0") {
					...
					$data_ok[4] .= '<a href="#" onclick="return m_siswa_non_aktif(' . $d['id'] . ');" class="btn btn-success btn-xs"><i class="glyphicon glyphicon-flash" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Disable User</a>';
				}
	...
	public function m_soal()
	{
		...
		if ($uri3 == "det") {
			...
			foreach ($q_datanya as $d) {
				...
				$data_ok[2] = $d['nmguru'] . '<br>' . $d['nmkelas'] . '/' . $d['nmmapel'];
				$data_ok[3] = "Dipakai : " . ($total) . ", Benar: " . $jml_salah . ", Salah: " . $jml_salah . "<br>Persentase benar : " . number_format($persen_benar) . " %";
				$data_ok[4] = '<div class="btn-group">
				  <a href="' . base_url() . 'adm/m_soal/edit/' . $d['id'] . '" class="btn btn-primary btn-xs"><i class="glyphicon glyphicon-pencil" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Edit</a>
	...
	public function h_ujian()
	{
		...
		if ($uri3 == "det") {
			...
			foreach ($q_datanya as $d) {
				$data_ok = array();
				$data_ok[0] = $no++;
				$data_ok[1] = "[" . $d['kelas'] . "] - " . "[" . $d['jurusan'] . "] - " . $d['nama_ujian'];
				$data_ok[2] = $d['nama_guru'];
				$data_ok[3] = $d['mapel'];
				$data_ok[4] = $d['jumlah_soal'];
				$data_ok[5] = $d['waktu'] . " menit";
				$data_ok[6] = '<a href="' . base_url() . 'adm/h_ujian/det/' . $d['id'] . '" class="btn btn-primary btn-xs"><i class="glyphicon glyphicon-search" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Lihat Hasil</a>
                         ';
	...
	public function act_login()
	{
		...
		if ($j_data === 1) {
			...
			$_log['log']['status']			= "0";
			$_log['log']['keterangan']		= "username dan password tidak ditemukan";
